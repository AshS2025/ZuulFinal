...
//Exit set ups
  ...
  Bank->setExit(West, Plaza);
  Bank->setExit(South, Parking);
  Bank->setExit(North, NULL);
  Bank->setExit(East, NULL);

  Salon->setExit(South, Restaurant);
  Salon->setExit(East, Castle);
  Salon->setExit(West, NULL);
  Salon->setExit(North, NULL);
 ...
 Castle->setExit(West, Salon);
  ...
  Arboretum->setExit(East, NULL);
  Arboretum->setExit(South, NULL);
  Arboretum->setExit(North, NULL);

  Dungeon->setExit(South, Castle);
  Dungeon->setExit(North, NULL);
  Dungeon->setExit(East, NULL);
  Dungeon->setExit(West, NULL);
...
void move(Room* &currentRoom) {
    ...
    //try catch
    if (currentRoom->getExit(move) == NULL){
      cout << "there is no room in that direction. type in another command." << endl;
    }

    //resetting the room you're in
    if (currentRoom->getExit(move) != NULL){
...
void get(Room* &currentRoom, vector<Item> &inventory) {
    ...
    if (currentRoom->getItem(item) == NULL){
      cout << "that item does not exist. type in GET again or another command" << endl;
